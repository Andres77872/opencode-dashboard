...
// TestSchemaInfoDerivation verifies the derivation logic in detectSchema
func TestSchemaInfoDerivation(t *testing.T) {
	tests := []struct {
		name                string
		hasSession          bool
		hasMessage          bool
		...
		hasWorkspace        bool
		hasPart             bool
		expectIsValidResult bool
	}{
		{name: "all present", hasSession: true, hasMessage: true, hasProject: true, hasWorkspace: true, hasPart: true, expectIsValidResult: true},
		{name: "missing session", hasSession: false, hasMessage: true, hasProject: true, hasWorkspace: true, hasPart: true, expectIsValidResult: false},
		{name: "missing message", hasSession: true, hasMessage: false, hasProject: true, hasWorkspace: true, hasPart: true, expectIsValidResult: false},
		{name: "missing project", hasSession: true, hasMessage: true, hasProject: false, hasWorkspace: true, hasPart: true, expectIsValidResult: false},
		{name: "missing workspace", hasSession: true, hasMessage: true, hasProject: true, hasWorkspace: false, hasPart: true, expectIsValidResult: false},
		{name: "missing part", hasSession: true, hasMessage: true, hasProject: true, hasWorkspace: true, hasPart: false, expectIsValidResult: false},
		{name: "all missing", hasSession: false, hasMessage: false, hasProject: false, hasWorkspace: false, hasPart: false, expectIsValidResult: false},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Replicate the derivation logic from detectSchema
			isValid := tt.hasSession && tt.hasMessage && tt.hasProject && tt.hasWorkspace && tt.hasPart

			if isValid != tt.expectIsValidResult {
				t.Errorf("derivation: all tables present = %v, but IsValid should be %v",
					tt.hasSession && tt.hasMessage && tt.hasProject && tt.hasWorkspace && tt.hasPart,
					tt.expectIsValidResult)
			}
		})
	}
}

func TestRequiredTables(t *testing.T) {
	// Verify requiredTables contains all expected OpenCode tables
	expectedTables := []string{"session", "message", "project", "workspace", "part"}
	...
	for _, expected := range expectedTables {
		...
		if !found {
			t.Errorf("requiredTables missing expected table: %q", expected)
		}
	}

	// Ensure no extra tables
	if len(requiredTables) != len(expectedTables) {
		t.Errorf("requiredTables has %d entries, want %d", len(requiredTables), len(expectedTables))
	}
}
