...
func redactConfigValue(key string, value any) (any, bool) {
	...
	case []map[string]any:
		...
		changed := false
		for i, item := range v {
			redacted, itemChanged := redactConfigMapWithParent(item, key)
			out[i] = redacted
			changed = changed || itemChanged
		}
		return out, changed
...
func encodeRedactedTOML(content map[string]any) (string, error) {
...
func sanitizeParseError(err error) string {
	...
	return fmt.Sprintf("invalid TOML: %s", text)
}
