...
func (s *snapshot) overview(pq stats.PeriodQuery) (stats.OverviewStats, error) {
	...
	totalCost, tokens, status, provenance := aggregateCostProvenance(messages)
	sessions := uniqueSessions(messages)
	...
	if result.Days > 0 {
		result.CostPerDay = totalCost / float64(result.Days)
	}
	return result, nil
}
...
func (s *snapshot) daily(pq stats.PeriodQuery, granularity ...stats.Granularity) (stats.DailyStats, error) {
	...
	return stats.DailyStats{SourceID: claudeSourceID, Days: days, Granularity: gran, CostStatus: status, CostProvenance: provenance}, nil
}
...
func (s *snapshot) dailyDimension(dimension string, pq stats.PeriodQuery, granularity ...stats.Granularity) (stats.DailyDimensionStats, error) {
	if dimension != "model" && dimension != "tool" && dimension != "project" {
		return stats.DailyDimensionStats{}, stats.InvalidDimensionError(dimension, "model, tool, project")
	}
	gran := stats.ResolveGranularity(pq, granularity...)
	messages, err := s.filteredMessages(pq)
	if err != nil {
	...
	_, _, status, provenance := aggregateCostProvenance(messages)
	return stats.DailyDimensionStats{SourceID: claudeSourceID, Days: days, Dimension: dimension, Period: periodLabel, Granularity: gran, CostStatus: status, CostProvenance: provenance}, nil
}
...
func (s *snapshot) models(pq stats.PeriodQuery) (stats.ModelStats, error) {
	...
	for key, agg := range aggs {
		...
		entry := stats.ModelEntry{
			...
			Messages:       int64(len(agg.messages)),
			Cost:           cost,
			Tokens:         tokens,
			CostStatus:     status,
			CostProvenance: provenance,
		...
		if entry.Messages > 0 {
			entry.AvgTokensPerMessage = &stats.AvgTokenStats{
				Input:      float64(tokens.Input) / float64(entry.Messages),
		...
		if entry.Sessions > 0 {
			entry.AvgTokensPerSession = &stats.AvgTokenStats{
				Input:      float64(tokens.Input) / float64(entry.Sessions),
				Output:     float64(tokens.Output) / float64(entry.Sessions),
				Reasoning:  float64(tokens.Reasoning) / float64(entry.Sessions),
				CacheRead:  float64(tokens.Cache.Read) / float64(entry.Sessions),
				CacheWrite: float64(tokens.Cache.Write) / float64(entry.Sessions),
			}
		}
	...
	_, _, status, provenance := aggregateCostProvenance(messages)
	return stats.ModelStats{SourceID: claudeSourceID, Models: models, CostStatus: status, CostProvenance: provenance}, nil
}
...
func (s *snapshot) tools(pq stats.PeriodQuery) (stats.ToolStats, error) {
	...
	for _, msg := range messages {
		for _, tool := range msg.ToolParts {
			...
			agg := aggs[tool.Tool]
			if agg == nil {
				agg = &aggregate{entry: stats.ToolEntry{SourceID: claudeSourceID, Name: tool.Tool}, sessions: map[string]bool{}}
				aggs[tool.Tool] = agg
			}
	...
	return stats.ToolStats{SourceID: claudeSourceID, Tools: tools}, nil
}
...
func (s *snapshot) projects(pq stats.PeriodQuery) (stats.ProjectStats, error) {
	...
	for id, group := range byProject {
		...
		if project == nil {
			project = &projectRecord{ID: id, Name: projectName(id, "")}
		}
		cost, tokens, status, provenance := aggregateCostProvenance(group)
		projects = append(projects, stats.ProjectEntry{SourceID: claudeSourceID, ProjectID: id, ProjectName: project.Name, Sessions: int64(len(uniqueSessions(group))), Messages: int64(len(group)), Cost: cost, Tokens: tokens, CostStatus: status, CostProvenance: provenance})
	}
	...
	_, _, status, provenance := aggregateCostProvenance(messages)
	return stats.ProjectStats{SourceID: claudeSourceID, Projects: projects, CostStatus: status, CostProvenance: provenance}, nil
}
...
func (s *snapshot) projectByID(id string, pq stats.PeriodQuery, page, limit int) (*stats.ProjectDetail, error) {
	...
	return &stats.ProjectDetail{SourceID: claudeSourceID, ProjectID: id, ProjectName: project.Name, Worktree: project.Worktree, Sessions: int64(len(uniqueSessions(projectMessages))), Messages: int64(len(projectMessages)), Cost: cost, Tokens: tokens, RecentSessions: recentEntries, TotalSessions: int64(len(allSessions)), CostStatus: status, CostProvenance: provenance}, nil
}
...
func (s *snapshot) sessions(query stats.SessionQuery) (stats.SessionList, error) {
	...
	return stats.SessionList{SourceID: claudeSourceID, Sessions: entries, Total: int64(len(sessions)), Page: page, PageSize: pageSize, CostStatus: status, CostProvenance: provenance}, nil
}
...
func (s *snapshot) messages(pq stats.PeriodQuery, page, limit int, sortSpec stats.MessageSort) (stats.MessageList, error) {
	...
	_, _, status, provenance := aggregateCostProvenance(messages)
	return stats.MessageList{SourceID: claudeSourceID, Messages: entries, Total: int64(len(messages)), Page: page, PageSize: limit, CostStatus: status, CostProvenance: provenance}, nil
}
...
func (s *sessionRecord) entry() stats.SessionEntry {
	...
	return stats.SessionEntry{SourceID: claudeSourceID, ID: s.ID, Title: s.Title, ProjectID: s.ProjectID, ProjectName: s.ProjectName, TimeCreated: s.Created, TimeUpdated: s.Updated, MessageCount: int64(len(s.Messages)), Cost: cost, CostStatus: status, CostProvenance: provenance}
}
